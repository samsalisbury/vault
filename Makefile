...
# This is the key required to sign stage and publish commits.
...
stage-commit:
	...
	$(GPG_KEY_VARS) && git commit --allow-empty --gpg-sign=$$GIT_GPG_KEY_ID -m 'release: stage v$(STAGE_VERSION)' 

publish-commit:
	$(FAIL_IF_GPG_KEY_MISSING)
	$(FAIL_IF_UNCOMMITTED_CHANGES)
	@[ -n "$(PUBLISH_VERSION)" ] || { echo "You must set PUBLISH_VERSION to the version in semver-like format."; exit 1; }
	@$(GPG_KEY_VARS) && git commit --allow-empty --gpg-sign=$$GIT_GPG_KEY_ID -m 'release: publish v$(PUBLISH_VERSION)' 
